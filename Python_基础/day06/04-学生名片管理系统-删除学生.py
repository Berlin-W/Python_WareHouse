...
def update_one_stu():
    ...
    for stu_dict in stu_list:
        ...
        # 如果学生不存在，提示：查无此人！没有该学生信息！
        print(f'查无此人！没有该学生{name}信息！')
...
def delete_one_stu():
    ...
    for i, stu_dict in enumerate(stu_list):
        if stu_dict['name'] == name:
            # 如果学生已存在，则将学生数据从 stu_list 中删除
            del stu_list[i]
            break
    else:
        # 如果学生不存在，就提示：查无此人！学生不存在！
        print(f'查无此人！学生{name}不存在！')
